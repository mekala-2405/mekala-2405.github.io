...
const TECH_ICON: Record<string, string> = {
  ...
  'ROS 2': 'devicon-ros-original colored',
};
...
function techIconHtml(tech: string): string {
  const dev = TECH_ICON[tech];
  if (dev) return `<i class="${dev}"></i>`;
...
function hideHoverPreview(): void {
  const el = byId('hoverPreview');
  el.classList.remove('visible');
  byId<HTMLIFrameElement>('hoverPreviewFrame').src = '';
}
...
function renderApp(): void {
  ...
  const projHtml = d.projects
    .map((p, i) => {
      ...
      const cardPreview = p.live ? `data-hover-preview="${p.live}"` : '';
      const projLinks = [
      ...
      return html`
        <div class="project-card reveal reveal-${(i % 4) + 1}" ${cardPreview}>
...
function afterRender(): void {
  ...
  // lazy-load project preview iframes on scroll + hover
  document.querySelectorAll<HTMLElement>('.project-card').forEach((card) => {
    card.addEventListener('mouseenter', () => loadPreviewFrame(card));
    new IntersectionObserver(
      (entries, obs) => {
        if (entries[0].isIntersecting) {
          loadPreviewFrame(card);
          obs.disconnect();
        }
      },
      { threshold: 0.4 }
  ...
  // hover preview tooltip for project names
  const previewEl = byId('hoverPreview');
  const previewFrame = byId<HTMLIFrameElement>('hoverPreviewFrame');
  document.querySelectorAll<HTMLElement>('[data-hover-preview]').forEach((el) => {
    el.addEventListener('mouseenter', () => {
      const url = el.dataset.hoverPreview;
      if (url) {
    ...
    el.addEventListener('mousemove', (e) => {
      ...
      previewEl.style.left = Math.min(x, window.innerWidth - 380) + 'px';
      previewEl.style.top = Math.min(y, window.innerHeight - 260) + 'px';
    });
  });
}
